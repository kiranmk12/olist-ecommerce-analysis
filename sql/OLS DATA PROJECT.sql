...
INSERT INTO FactOrders (
    ...
    revenue, freight_value, delivery_days, estimated_days, delay_days, is_late,
    review_score, payment_value, payment_type, order_year, order_month, order_date
)
SELECT
    o.order_id,
    dc.customer_key,
...
CategoryReviews AS (
    ...
    FROM (
        SELECT DISTINCT f.order_id, dp.product_category_english, f.review_score
        FROM FactOrders f
        LEFT JOIN DimProduct dp ON f.product_key = dp.product_key
    ) unique_orders
...
SELECT TOP 10
    ...
    ROUND(v.avg_review_score, 2) AS avg_review_score
FROM CategoryRevenue r
JOIN CategoryReviews v ON r.product_category_english = v.product_category_english
ORDER BY total_revenue DESC



--  CUSTOMER STATE PERFROMANCE


WITH UniqueOrdersByState AS (
    SELECT DISTINCT 
        f.order_id,
        dc.customer_state,
        f.delivery_days,
...
SELECT 
    ...
    ROUND(SUM(CAST(is_late AS FLOAT)) * 100.0 / COUNT(order_id), 1) AS late_percentage
FROM UniqueOrdersByState
...
SellerOrderMetrics AS (
    SELECT 
        seller_key,
        AVG(CAST(review_score AS FLOAT)) AS avg_review_score,
        AVG(CAST(delivery_days AS FLOAT)) AS avg_delivery_days
    FROM (
        SELECT DISTINCT order_id, seller_key, review_score, delivery_days
        FROM FactOrders
    ) unique_seller_orders
    GROUP BY seller_key
)
SELECT TOP 10
    ds.seller_id,
...
FROM SellerRevenue r
JOIN SellerOrderMetrics m ON r.seller_key = m.seller_key
LEFT JOIN DimSeller ds    ON r.seller_key = ds.seller_key
ORDER BY total_revenue DESC
...
SellerReviews AS (
    ...
    FROM (
        SELECT DISTINCT order_id, seller_key, review_score 
        FROM FactOrders
    ) unique_seller_orders
    GROUP BY seller_key
)
SELECT 
    ...
    ds.seller_state,
    ROUND(r.total_revenue, 2) AS total_revenue,
    ROUND(v.avg_score, 2)     AS avg_score,
    NTILE(4) OVER (
        ORDER BY r.total_revenue DESC
    ) AS revenue_quartile
FROM SellerRevenue r
JOIN SellerReviews v   ON r.seller_key = v.seller_key
LEFT JOIN DimSeller ds ON r.seller_key = ds.seller_key
ORDER BY total_revenue DESC
...
BEGIN
    WITH OrderSummary AS (
        SELECT 
            ...
            MAX(is_late) AS order_is_late   
        FROM FactOrders
        WHERE order_year = @Year
        GROUP BY order_id, order_year, order_month
    )
    SELECT 
        order_year,
        order_month,
        ...
        SUM(order_is_late)                AS late_orders,
        ROUND(SUM(CAST(order_is_late AS FLOAT)) * 100.0 / 
...
CREATE VIEW vw_OrderLogisticsSummary AS
WITH CleanOrders AS (
    SELECT DISTINCT 
        order_id,
        customer_key,
...
SELECT 
    ...
    o.delay_days,
    o.is_late,
    dbo.fn_DeliveryStatus(o.delay_days) AS delivery_status
FROM CleanOrders o
LEFT JOIN DimCustomer dc ON o.customer_key = dc.customer_key


select * from vw_OrderLogisticsSummary
